...
def _call_perplexity(query: str, system_prompt: str = None) -> Optional[Dict]:
    """Call Perplexity API for political news search or classification."""
    ...
    try:
        ...
        logger.error(f"Perplexity API error {e.response.status_code}: {e}")
        return None
...
def _call_perplexity_json(prompt: str) -> Optional[Dict]:
    """Call Perplexity and parse JSON response."""
    result = _call_perplexity(prompt, system_prompt="You are a political news analyst. Respond ONLY with valid JSON.")
    if not result:
        return None
...
# ── Apify Social Monitor ────────────────────────────────────────

def _apify_run_actor(actor_id: str, run_input: dict, timeout: int = 120) -> list:
    """Run an Apify actor and return results. Graceful fallback if no token."""
    ...
    try:
        ...
        with httpx.Client(timeout=timeout) as client:
            ...
            if resp.status_code == 402:
                logger.warning("Apify: insufficient credits")
                return []
...
def _check_political_twitter(queries: list, max_results: int = 20) -> list:
    """Search Twitter for political keywords."""
    all_results = []
    for q in queries[:5]:
        results = _apify_run_actor(
            "apidojo/tweet-scraper",
            {"searchTerms": [q], "maxTweets": max_results // len(queries[:5]),
             "sort": "Latest"},
        )
        for r in results:
            all_results.append({
...
def _check_political_facebook(accounts: list, max_results: int = 10) -> list:
    """Check Facebook pages for political posts."""
    all_results = []
